...
suite('index', function () {
  ...
  suite('Store', function () {
    ...
    var store = new Store('test/dat/numbers.store');
  ...
  suite('Store.createWriteStream', function () {
    ...
    test('empty', function (done) {
      ...
      var s = Store.createWriteStream(path, function () {
    ...
    test('metadata', function (done) {
      ...
      var s = Store.createWriteStream(path, opts, function () {
        var store = new Store(path);
    ...
    test('invalid entry', function (done) {
      ...
      var key = new Buffer([1]);
      var numErrors = 0;
      var s = Store.createWriteStream(path)
    ...
    test('collision', function (done) {
      ...
      var s = Store.createWriteStream(path, opts, function (err) {
        assert.strictEqual(err, null);
        getEntries(new Store(path), function (arr) {
    ...
    test('duplicate key', function (done) {
      var path = tmp.tmpNameSync();
      var key = new Buffer([1]);
      var s = Store.createWriteStream(path, function (err) {
        assert(err);
        setTimeout(function () {
          try {
            fs.statSync(path);
