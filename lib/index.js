...
Db.createWriteStream = function (path, opts, cb) {
  ...
  ts.pipe(store.Store.createWriteStream(path, opts, cb));
  return ts;
};
...
function AvroDb(path) {
  ...
  var opts = {
    codecs: {
      keyCodec: new codecs.AvroCodec(avsc.parse(metadata.keySchema)),
      valueCodec: new codecs.AvroCodec(avsc.parse(metadata.valueSchema))
    }
  };
...
AvroDb.createWriteStream = function (path, schemas, opts, cb) {
  if (typeof opts == 'function' && !cb) {
    cb = opts;
    opts = undefined;
  }
  ...
  var opts_ = {
    codecs: {
      keyCodec: new codecs.AvroCodec(avsc.parse(schemas.keySchema)),
      valueCodec: new codecs.AvroCodec(avsc.parse(schemas.valueSchema))
    },
    metadata: new Buffer(JSON.stringify(schemas))
  };
